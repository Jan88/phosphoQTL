...
nforest <- 100
exclude <- 3594:3600
load("/home/jgrossb1/phosphoQTL/phosphoProtResidualsQTL/phosphoProtResidualsQtlMappingData170831.RData")
...
AllScores <- parSapply(cl=cl,1:nrow(phenotype),FUN=function(x){
  ...
  scores <- rfMapper(phenotype = phenotype,
                     ...
                     nforest = nforest,
                     ntree = ntree)
  save(scores,file=paste("/scratch/jgrossb1/phosphoQTL/phosphoProtResidualsQTL/real/trait",x,".RData",sep=""))
  return(scores)
})
stopCluster(cl)
