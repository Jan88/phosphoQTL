pdfAndPng<-function(file, width, height,expr,Cairo=FALSE,...){
  if(Cairo){
    require(Cairo)
    pdfFUN<-CairoPDF
    pngFUN<-CairoPNG
  } else { pdfFUN<-pdf; pngFUN<-png}
  
  pdfFUN(paste(file,'.pdf',sep=''),width=width,height=height,...)
  eval(expr,parent.frame(n = 2))
  dev.off()
  pngFUN(paste(file,'.png',sep=''),width=width*100,height=height*100,pointsize=18,...)
  eval(expr,parent.frame(n = 2))
  dev.off()
}
