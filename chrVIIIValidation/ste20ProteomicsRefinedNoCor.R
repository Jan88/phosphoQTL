...
## Inputs
input      <- "ste20Validation/proteinData/E2001301608_feature_alignment.tsv"
...
## Outputs
trs        <- "ste20Validation/proteinData/proteinNoCor/transitions.csv"
trsmod     <- "ste20Validation/proteinData/proteinNoCor/transition_mod.csv"
output_trs <- "ste20Validation/proteinData/proteinNoCor/trslevel"
output_mapDIA <- "ste20Validation/proteinData/proteinNoCor/mapDIA/mapDIA_trslevel.tsv"
...
## Remove features with >=6 missing values (count entries in list - no NAs!)
...
# "2558 proteins before filtering"
# "2369 proteins after filtering"

## Refine file names and lookup corresponding culture IDs
rawRefRed$filename <- gsub(".*/", "", rawRefRed$filename)
rawRefRed$filename <- gsub("\\..*", "", rawRefRed$filename) 
cultureLookup      <- cbind(filename=as.character(meta$FileName.NonEnriched), culture=rownames(meta))
...
#modified because 1473-1477 differ only in GPA1/STE20
...
all(inRM==1)&all(inBY==1) #no need to remove anything
# sel<- peptideSeq%in%(uniquePeptideSeq[inBY==1 & inRM ==1])
# 
# dat<-dat[sel,]
...
bashcommand   <- paste("cd ste20Validation/proteinData/proteinNoCor/mapDIA/ && ./mapDIA input.params")
...
pLevel <- read.table("ste20Validation/proteinData/proteinNoCor/mapDIA/protein_level.txt",sep="\t", header=T)
p <- as.matrix(pLevel[,paste0("X",rownames(meta))])
p[p==0] <- NA
rownames(p)<-pLevel$Protein
p<-log2(p)


#only culture batches, all samples were measured together in the MS
batch <- rep(1:4,4)
...
# 
prot <- pComBat
save(prot,file="data/protLevelsReplacementStrainsNoCor.RData")
...
prot <- prot[,setdiff(colnames(prot),"X1473_r3")]

diffProt <- lapply(1:nrow(comparisons),FUN=function(i){
  s1 <- comparisons[i,1]
  s2 <- comparisons[i,2]
  fc <- rowMeans(prot[,grepl(s1,colnames(prot))],na.rm=T)-rowMeans(prot[,grepl(s2,colnames(prot))],na.rm=T)
  pv <- sapply(rownames(prot),FUN=function(p){
    s1Samples <- grepl(s1,colnames(prot))
    s2Samples <- grepl(s2,colnames(prot))
    if(sum(is.na(prot[p,s1Samples]))>1|sum(is.na(prot[p,s2Samples]))>1){return(NA)}
    t.test(prot[p,s1Samples],prot[p,s2Samples])$p.value
  })
  fdr <- p.adjust(pv,method="BH")
  out <- cbind(p=pv,fdr=fdr,l2FC=fc)
  rownames(out) <- rownames(prot)
  return(out)
})

save(diffProt,comparisons,file="data/diffProtReplacementStrainsNoCor.RData")

#peptide levels after mapdia

p <- read.table("/cellnet/phosphoQTL/ste20Validation/proteinData/proteinNoCor/mapDIA/peptide_level.txt",sep="\t",header=T,as.is=T)
rownames(p) <- p$Peptide
pep2Prot <- p[,1:2]
p <- p[,grepl("X",colnames(p))]
p[p==0] <- NA
p<-log2(p)
...
pComBat = ComBat(dat=p, batch=batch, mod=mod, par.prior=F, prior.plots=F)
pepLevels <- pComBat
pep2Prot <- pep2Prot[sel1,]
save(pepLevels,pep2Prot,file="data/pepLevelsReplacementStrainsNoCor.RData")
